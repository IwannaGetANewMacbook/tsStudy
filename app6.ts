// 03.03
{
  ...
  let animal2: Animal2 = {name: "lion", age: 12}
}

{
  ...
  const name: Person = "타입변수 합치기 기능 수행 완료."
...
그건 여러분들이 한 번 테스트해보길 바랍니다.`)
  ...
  type Object2 = {name: string, region: string}
...
Type alias로 만들어 보셈.`)
  ...
  type Object = {color? : string, size: number, readonly position: readonly number[]}
  const object: Object = {
    color: "red",
    size: 19,
    position: [12, 13]
...
3. 각 속성이 어떤 타입일지는 자유롭게 정하십시오. `)
  ...
  const user: User = { name : 'kim', phone : 123, email : 'abc@naver.com' }
...
3. 멋있게 숙제2에서 만들어둔  type alias를 재활용해봅시다.`)
  ...
  type User = {name: string, phone: number, email?: string}
  const user: User & {isAdult: boolean} = {
    name: "kimsuhyeon",
    phone: 57557503,
