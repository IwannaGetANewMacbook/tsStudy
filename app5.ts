...
// <Narrowing & Assertion>
{
  ...
  function myFunction(x: number | string): number {
    // Type Narrowing 예제1
    if(typeof x === "number") {
      return x * x
    } else {
...

{
  ...
  function myFunction3(x: number | string) {
    ...
    // 이것이 바로 "Assertion!!"
    arr[0] = x as number    // 의미-"왼쪽에 있는 변수를 number로 덮어 써주세용!"
  }
  console.log(
`빠다 안맞기 위한 assertion(as) 문법의 용도.
...
{
  console.log("\n","(숙제1)")
  function cleanForNumber(x: (string | number)[]) {
    let list: number[] = []
    x.forEach((v, i, a) => {
      if(typeof v === "string") {
        list.push(Number(v))
...
{
  console.log("\n","(숙제2)")
  type www = {subject: string | string[] }

  let ChulSu_t: www = {subject: "math"}
  let YoungHee_t: www = {subject: ["science", "english"]}
  let MinSu_t: www = {subject: ["science", "art", "korean"]}

  function lastSubReturn (ob: www) {
