...
숫자 또는 문자 이외의 자료가 들어오면 안됩니다.
  ...
  function homework2(x: number | string) {
    if(typeof(x) === "number") {
...
`)
  ...
  function canIgetMerried(wage: number, myHouse: boolean, attractiveP: string): string | void {
    if(wage < 0) {
      console.log("월 소득을 똑바로 입력하십시오.")
      return
    }
    const wagePoint: number = wage * 1
    let housePoint: number
    let attPoint: number
    ...
    const totalPoint: number = wagePoint + housePoint + attPoint
    if (totalPoint >= 600) {
      return  "결혼가능"
    }
  }
